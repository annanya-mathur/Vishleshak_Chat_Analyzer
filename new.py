...
def display_statistics(stats):
    ...
    st.markdown('<p style="font-size:30px; line-height:1.5;">{:.2f}</p>'.format(stats['messages_per_day']), unsafe_allow_html=True)
...
def display_emojis(df):
    ...
    st.markdown('<p style="font-size:20px; line-height:1.5;">{}</p>'.format(', '.join(unique_emojis)), unsafe_allow_html=True)
